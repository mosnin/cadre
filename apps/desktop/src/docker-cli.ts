...
import type { DesktopSetupLink } from "@cadre/contracts";
...
/**
 * `CADRE_DOCKER_BINARY` is a test hook and the only candidate when set, so a
...
export function resolveDockerBinary(
  ...
  const override = env.CADRE_DOCKER_BINARY?.trim();
