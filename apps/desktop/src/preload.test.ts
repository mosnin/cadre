...
import type { CadreDesktop, CadreSetup } from "@cadre/contracts";
...
describe("desktop preload bridge", () => {
  it("exposes only the platform, the four window operations, the updater, and the OAuth bridge", async () => {
    ...
    const [globalName, bridge] = exposeInMainWorld.mock.calls[0] as [string, CadreDesktop];
    expect(globalName).toBe("cadreDesktop");
  ...
  it("forwards captured codes without leaking the IPC event to the renderer", () => {
    ...
    const [, bridge] = exposeInMainWorld.mock.calls[0] as [string, CadreDesktop];
...
describe("setup preload bridge", () => {
  it("exposes only the first-run setup operations", async () => {
    ...
    expect(exposeInMainWorld).toHaveBeenCalledTimes(1);
    const [globalName, bridge] = exposeInMainWorld.mock.calls[0] as [string, CadreSetup];
    expect(globalName).toBe("cadreSetup");
    expect(bridge.platform).toBe("linux");
