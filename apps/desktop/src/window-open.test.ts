...
const appOrigin = "https://cadre.example.com";
...
describe("desktop child windows", () => {
  ...
  it("opens ordinary external links outside Electron", () => {
    expect(
      shouldOpenInAppPopup(appOrigin, "https://github.com/mosnin/cadre/pull/395", "_blank"),
    ).toBe(false);
  });

  it.each(["cadre-model-oauth", "cadre-mcp-oauth", "cadre-app-connect", "cadre-plugin-connect"])(
  ...
  it("rejects malformed URLs and non-HTTPS third-party targets", () => {
    expect(shouldOpenInAppPopup(appOrigin, "not a url", "cadre-model-oauth")).toBe(false);
    expect(
      shouldOpenInAppPopup(appOrigin, "http://provider.example.com", "cadre-model-oauth"),
    ).toBe(false);
  });
});
