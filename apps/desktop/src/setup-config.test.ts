...
describe("server address normalization", () => {
  it("assumes http locally and https for a bare public host", () => {
    ...
    expect(normalizeServerUrl("cadre.example.com")).toBe("https://cadre.example.com");
  });

  it("keeps an explicit secure scheme and port but stores only the origin", () => {
    expect(normalizeServerUrl("https://cadre.example.com")).toBe("https://cadre.example.com");
    expect(normalizeServerUrl("https://cadre.example.com:8443/team")).toBe(
      "https://cadre.example.com:8443",
    );
  });
  ...
  it("rejects cleartext public servers but permits private-network development", () => {
    expect(normalizeServerUrl("http://cadre.example.com")).toBeNull();
  ...
  it("rejects cleartext link-local addresses used by cloud metadata endpoints", () => {
    ...
    // HTTPS to link-local still normalizes; the health probe must match Cadre.
  ...
  it.each(["", "   ", "not a url", "ftp://example.com", "file:///etc/passwd", "http://"])(
    "rejects an address that cannot reach a Cadre server (%s)",
    (value) => {
  ...
  it("rejects embedded credentials rather than writing them to disk", () => {
    expect(normalizeServerUrl("https://user:secret@cadre.example.com")).toBeNull();
  });
});
...
describe("managed local open URL", () => {
  ...
  it("rejects a different loopback origin even when both are local", () => {
    ...
    expect(managedLocalOpenUrl("https://cadre.example.com", DEFAULT_LOCAL_WEB_URL)).toBeNull();
  });
});

describe("desktop stack token transport", () => {
  it("allows HTTPS and loopback HTTP only", () => {
    expect(maySendDesktopStackToken("https://cadre.example.com")).toBe(true);
    ...
    expect(maySendDesktopStackToken("http://cadre.local:5173")).toBe(false);
  });
});

describe("saved setup", () => {
  it("round-trips through the on-disk format", () => {
    const setup = { mode: "existing", serverUrl: "https://cadre.example.com" } as const;
...
describe("startup target", () => {
  const saved = { mode: "existing", serverUrl: "https://cadre.example.com" } as const;
  ...
  it("opens the saved instance on later launches", () => {
    expect(resolveStartupTarget({ saved })).toEqual({
      kind: "app",
      url: "https://cadre.example.com",
      source: "saved",
    });
  });

  it("lets CADRE_WEB_URL point the shell anywhere without touching saved setup", () => {
  ...
  it("ignores an empty CADRE_WEB_URL", () => {
...
describe("bundled renderer eligibility", () => {
  it("stands in for http(s) origins only", () => {
    ...
    expect(servesBundledRenderer("https://cadre.example.com")).toBe(true);
...
describe("remote-content isolation", () => {
  it("uses a stable, opaque session partition per server origin", () => {
    ...
    expect(first).toMatch(/^persist:cadre-[a-f0-9]{24}$/);
...
describe("Cadre health response", () => {
  it("requires the public RPC health contract", () => {
    expect(isCadreHealth({ json: { ok: true, version: "0.1.0" } })).toBe(true);
    expect(isCadreHealth({ json: { ok: true } })).toBe(false);
    expect(isCadreHealth({ ok: true, version: "0.1.0" })).toBe(false);
  });
});
