import type { CadreDesktopOAuthCallback } from "@cadre/contracts";
 ...
 * in the redirect URL. Cadre runs no listener on that port and asks for the
...
): CadreDesktopOAuthCallback | undefined {
  let target: URL;
  try {
    target = new URL(url);
  } catch {
