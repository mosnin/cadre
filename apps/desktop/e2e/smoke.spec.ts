...
import type { CadreDesktop } from "@cadre/contracts";
import { _electron as electron, expect, test } from "@playwright/test";

const fixture = `<!doctype html>
<html lang="en">
  <head><meta charset="utf-8"><title>Cadre desktop smoke</title></head>
...
test("launches with a narrow preload bridge and an isolated renderer", async () => {
  const app = await electron.launch({
    ...
    env: {
      ...process.env,
      CADRE_WEB_URL: `data:text/html;charset=utf-8,${encodeURIComponent(fixture)}`,
    },
  });
  ...
  try {
    ...
    await expect(page).toHaveTitle("Cadre desktop smoke");

    const renderer = await page.evaluate(async () => {
      const desktop = (window as typeof window & { cadreDesktop?: CadreDesktop }).cadreDesktop;
