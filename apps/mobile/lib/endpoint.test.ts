...
describe("normalizeApiBase", () => {
  it("trims, adds https, and keeps only the origin", () => {
    ...
    expect(normalizeApiBase("https://cadre.example.com:8443/api/")).toEqual({
      ok: true,
      url: "https://cadre.example.com:8443",
    });
...
describe("display and warnings", () => {
  ...
  it("shows host and non-default port", () => {
    expect(displayApiHost("https://cadre.example.com")).toBe("cadre.example.com");
  ...
  it("treats the compile-time default as not custom", () => {
    ...
    expect(usesCustomApiBase("https://cadre.example.com")).toBe(true);
  });
});

describe("probeApiBase", () => {
  it("accepts a Cadre /rpc/health response", async () => {
  ...
  it("rejects a host that is up but is not Cadre", async () => {
    const fetchImpl = vi.fn(
