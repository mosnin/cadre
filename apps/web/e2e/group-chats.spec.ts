...
test("create group from + and see two bots in one transcript", async ({ page }, testInfo) => {
  ...
  await signup(page, `group-${stamp}@cadre.test`, "password12", "Group E2E");
  ...
  await expect(groupAvatar.locator(".cadre-bot-avatar")).toHaveCount(2);
  const workingAvatar = groupAvatar.locator('[data-working="true"]');
  await expect(workingAvatar).toHaveCount(1);
  await expect(workingAvatar.locator("svg")).toHaveCSS("animation-name", "cadre-avatar-spin");
  ...
  await expect
    .poll(
      async () => {
        const snapshot = await rpc<{ members?: Array<{ botId: string; status?: string }> }>(
          page,
          "threads/get",
          { groupId: draftGroupId },
        );
        return snapshot.members?.find((member) => member.botId === writerId)?.status;
      },
      { timeout: 30_000 },
    )
    .toBe("waiting_input");
