...
export type WorkspaceLibraryStatus = { title: string | null; busy: boolean };

/** Plugin bundles plus skills and workflows, with their editor. Shared by the library dialog and the Skills tab. */
export function WorkspaceLibraryPanel({
  onChange,
  onStatus,
}: {
  onChange: () => void;
  /** Reports the open editor's title and whether a save is in flight, for the host surface. */
  onStatus?: (status: WorkspaceLibraryStatus) => void;
}) {
  const { t } = useLingui();
  const fieldId = useId();
  ...
  const title = editor ? (editor === "new" ? t`New skill or workflow` : editor.name) : null;
  useEffect(() => {
    onStatus?.({ title, busy });
  }, [title, busy]);
  return (
        ...
        <div className="space-y-6">
          <label htmlFor={`${fieldId}-plugin`} className="block space-y-2">
            ...
            <Input
              ...
              maxLength={120}
              disabled={busy}
              onChange={(event) => setPendingPlugin({ ...pendingPlugin, name: event.target.value })}
            />
          </label>
          <p className="text-sm text-muted-foreground">
            <Trans>
              {pendingPlugin.files.length} files will be saved in this workspace. Hooks and servers
              are not activated by importing.
            </Trans>
          </p>
            ...
            onChange={(event) => {
              ...
              void run(async () => {
                if (
                  ...
                  throw new Error(t`Choose a plugin folder with at most 5000 text files and 20 MB`);
