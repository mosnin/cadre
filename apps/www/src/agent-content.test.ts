...
describe("agent content negotiation", () => {
  ...
  it("maps canonical and trailing-slash page paths to Markdown documents", () => {
    expect(getMarkdownDocument("/")).toContain("# Cadre");
    expect(getMarkdownDocument("/about/")).toContain("# About Cadre");
  ...
  it("publishes specific when-to-use instructions for agents", () => {
    ...
    expect(AGENT_INSTRUCTIONS).toContain("## When to use Cadre");
    expect(AGENT_INSTRUCTIONS).toContain("## How an agent should use Cadre");
  ...
  it("returns cache-safe Markdown responses and omits bodies for HEAD", async () => {
    const response = markdownResponse("# Cadre\n");
    ...
    await expect(response.text()).resolves.toBe("# Cadre\n");

    const headResponse = markdownResponse("# Cadre\n", "HEAD", 404);
