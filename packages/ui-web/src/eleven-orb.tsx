/**
 * ElevenLabs' Orb, vendored.
 *
 *   https://ui.elevenlabs.io/r/orb.json — MIT, (c) 2025 Eleven Labs Inc.
 *   Notice in `eleven-orb.LICENSE`.
 *
 * Everything below is theirs and unchanged except for one line: the perlin
 * texture is served from our own `public/` rather than fetched from
 * `storage.googleapis.com` on every mount. A product that reaches a third
 * party's CDN to draw an avatar is one outage away from drawing nothing,
 * and it tells that CDN who is looking at the app.
 *
 * Do not reformat this file and do not tidy it. It is upstream's, and the
 * next version of it is a re-fetch, not a merge. Biome skips it for the
 * same reason (`biome.json` → `files.includes`).
 *
 * `@ts-nocheck` is not laziness: every error this repo raises against the
 * file is `noUncheckedIndexedAccess` firing on shader uniform lookups
 * (`u.uTime`, `u.uColor1`), a flag upstream does not set. The choice is
 * between editing 40 lines of someone else's shader plumbing — and owning
 * that edit forever — or declaring the file foreign. It is foreign.
 */
// @ts-nocheck
"use client"
...
/** Vendored: see the note at the top of this file. */
export const PERLIN_TEXTURE_URL = "/orb-perlin-noise.png"
