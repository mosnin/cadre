...
import { EASE_OUT, SPRING_LAYOUT, SPRING_PRESS } from "./support/ease";
...
const _UPLOAD_PROGRESS_MS = 900;
const _UPLOAD_COMPLETE_HOLD_MS = 1000;
const _REMOVE_PENDING_MS = 420;

const _WAVEFORM_BARS = [
...
function _formatDuration(seconds: number | undefined) {
...
function RowAction({
  ...
  if (state === "uploading") {
    return <span aria-hidden="true" className="size-11 shrink-0" />;
  }
    ...
    return (
      <Tooltip content="Upload complete" side="top" delay={100}>
        <motion.span
          ...
          className="grid size-11 shrink-0 place-items-center rounded-xl text-success"
        >
    ...
    return (
      <Tooltip content="Removing attachment" side="top" delay={100}>
        <span
          ...
          className="grid size-11 shrink-0 place-items-center rounded-xl text-muted-foreground"
        >
          <motion.span
      ...
      return (
        <Tooltip content="Upload failed" side="top" delay={100}>
          <span
            ...
            className="grid size-11 shrink-0 place-items-center rounded-xl text-destructive"
          >
    ...
    return (
      <Tooltip content="Retry upload" side="top" delay={100}>
        <motion.button
          ...
          className="grid size-11 shrink-0 place-items-center rounded-xl text-destructive outline-none transition-colors hover:bg-destructive/10 focus-visible:ring-2 focus-visible:ring-ring"
        >
  ...
  return (
    <Tooltip content="Remove attachment" side="top" delay={100}>
      <motion.button
        ...
        whileTap={undefined}
        transition={SPRING_PRESS}
        className="grid size-11 shrink-0 place-items-center rounded-xl text-muted-foreground outline-none transition-colors hover:bg-muted hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring"
      >
        <X className="size-4" />
      </motion.button>
    </Tooltip>
  ...
  return (
    ...
    >
      <motion.button
        ...
        whileTap={undefined}
        transition={SPRING_PRESS}
        className="group/image relative size-11 shrink-0 overflow-hidden rounded-[10px] bg-muted outline-none ring-1 ring-border/70 focus-visible:ring-2 focus-visible:ring-ring"
      >
        ...
        {({ isPresent, gate }) => (
          ...
          >
            <motion.button
              ...
              tabIndex={-1}
              className="pointer-events-auto fixed inset-0 size-full cursor-default bg-background/80 backdrop-blur-xl"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            <div className="fixed inset-4 flex items-center justify-center sm:inset-8">
              ...
              >
                ...
                <motion.button
                  ...
                  whileTap={undefined}
                  transition={SPRING_PRESS}
                  className="absolute -right-3 -top-3 grid size-11 place-items-center rounded-full bg-background text-foreground shadow-xl outline-none ring-1 ring-border/70 transition-colors hover:bg-muted focus-visible:ring-2 focus-visible:ring-ring"
                >
                  <X className="size-4" />
                </motion.button>
              </motion.div>
...
function AttachmentRow({
  ...
  const uploadProgress = uploading ? (
    <span role="status" aria-label={`Uploading ${item.name}`} className="sr-only">
      Uploading {item.name}
    </span>
  ) : null;
      ...
      <div className="relative isolate flex min-w-0 flex-1 items-center gap-3 self-stretch overflow-hidden rounded-xl bg-background px-2 py-1">
        ...
        {item.kind === "link" && item.href ? (
          <a
            ...
            className="grid size-11 shrink-0 place-items-center rounded-lg text-muted-foreground outline-none transition-colors hover:bg-muted hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring"
          >
...
export function AttachmentUpload({
  ...
  const [removingIds, _setRemovingIds] = useState<Set<string>>(() => new Set());
  ...
  return (
    <LayoutGroup id={inputId}>
      <div data-slot="attachment-upload" className={cn("w-full", className)}>
        {showDropzone ? (
          <>
            <input
              ref={inputRef}
        ...
        {items.length > 0 ? (
          <section
            className={showDropzone ? "mt-8" : ""}
            aria-labelledby={`${inputId}-attachments`}
          >
            <h3
              id={`${inputId}-attachments`}
              className={showDropzone ? "text-sm font-semibold text-foreground" : "sr-only"}
            >
              {attachmentsLabel}
            </h3>

            {items.length > 0 ? (
              <ul className={cn(showDropzone ? "mt-3 space-y-2" : "space-y-2", classNames?.list)}>
