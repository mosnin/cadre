...
export function ComboboxTrigger({ children, className }: ComboboxTriggerProps) {
  ...
  return (
    <div
      ...
      className={cn(
        "relative z-20 flex min-h-11 w-full min-w-0 cursor-text items-center justify-between gap-3 rounded-xl border border-border bg-transparent px-3 text-sm text-foreground transition-[border-color] hover:border-(--color-border-strong)",
  ...
  return (
    <div className={cn("flex min-w-0 flex-1 items-center gap-2", wrapperClassName)}>
      ...
      <input
        ...
        className={cn(
          "min-h-11 min-w-0 flex-1 bg-transparent text-sm text-foreground outline-none placeholder:text-muted-foreground disabled:cursor-not-allowed",
          className,
        )}
      />
    </div>
