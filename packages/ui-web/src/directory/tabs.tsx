...
const transition: Transition = { duration: 0.16, ease: EASE_OUT };
  ...
  if (variant === "underline") {
    return (
      <button
        ...
        tabIndex={active ? 0 : -1}
        id={`${layoutId}-tab-${encodeURIComponent(value)}`}
        aria-controls={`${layoutId}-panel-${encodeURIComponent(value)}`}
        onClick={() => setValue(value)}
        className={cn(
          "relative isolate px-1 pb-2.5 pt-1 -mb-px text-sm font-medium transition-colors min-h-[44px] inline-flex items-center",
  ...
  return (
    <div className="relative flex-1">
      ...
      <button
        ...
        onClick={() => setValue(value)}
        className={cn(
          "relative z-10 inline-flex items-center justify-center whitespace-nowrap min-h-11 bg-transparent px-3.5 py-2.5 text-sm font-medium outline-none",
  ...
  if (!active) {
    return (
      <div
        hidden
        role="tabpanel"
        id={`${layoutId}-panel-${encodeURIComponent(value)}`}
        aria-labelledby={`${layoutId}-tab-${encodeURIComponent(value)}`}
        className={className}
      >
        {children}
      </div>
