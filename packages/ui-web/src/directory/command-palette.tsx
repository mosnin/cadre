...
import { searchCommands } from "./support/command-search";
import { EASE_OUT } from "./support/ease";
import { useOnOpen } from "./support/hooks/use-on-open";
import { useRowCursor } from "./support/hooks/use-row-cursor";
import { useTouchCapable } from "./support/hooks/use-touch-capable";
...
export function CommandPalette({
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (shortcut && (e.metaKey || e.ctrlKey) && e.key.toLowerCase() === shortcut.toLowerCase()) {
        e.preventDefault();
        setOpen(!open);
        return;
      }
  ...
  const onKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === "Tab") {
      ...
      const item = rows[Number(e.key) - 1];
      if (item) {
        e.preventDefault();
        item.onSelect();
        setOpen(false);
                        ...
                        {list.map((it) => {
                          ...
                          return (
                            <button
                              ...
                              className={cn(
                                "relative isolate flex min-h-11 w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm",
