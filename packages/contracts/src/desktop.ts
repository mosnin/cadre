...
export interface CadreDesktopUpdate {
...
export interface CadreDesktopOAuthCallback {
  code: string;
  state?: string;
}

export interface CadreDesktop {
  platform: string;
  window: {
    close: () => Promise<void>;
  ...
  update: CadreDesktopUpdate;
  oauth: {
    /**
     * Authorization codes captured from a sign-in popup's loopback redirect.
     * Returns an unsubscribe function.
     */
    onCallback: (listener: (callback: CadreDesktopOAuthCallback) => void) => () => void;
  };
}

/**
 * How the desktop app was pointed at a Cadre server during first-run setup.
 ...
 * narrower `cadreDesktop` bridge so a connected server can never re-point the app.
 */
export interface CadreSetup {
