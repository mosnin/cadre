...
describe("normalizeRepoUrl", () => {
  it("accepts the official repository and https forks", () => {
    expect(normalizeRepoUrl(OFFICIAL_REPO_URL)).toEqual({
      url: "https://github.com/mosnin/cadre",
    });
    expect(normalizeRepoUrl("  https://github.com/me/cadre.git/  ")).toEqual({
      url: "https://github.com/me/cadre.git",
    });
    expect(normalizeRepoUrl("https://git.example.com:8443/team/cadre.git")).toEqual({
      url: "https://git.example.com:8443/team/cadre.git",
    });
  });

  it("accepts ssh remotes in both spellings", () => {
    expect(normalizeRepoUrl("git@github.com:me/cadre.git")).toEqual({
      url: "git@github.com:me/cadre.git",
    });
    expect(normalizeRepoUrl("ssh://git@github.com/me/cadre.git")).toEqual({
      url: "ssh://git@github.com/me/cadre.git",
    });
  });
  ...
  it("refuses transports and shapes that are not a git remote", () => {
    for (const input of [
      "",
      "   ",
      "http://github.com/me/cadre",
      "git://github.com/me/cadre",
      "file:///etc/passwd",
      "/srv/cadre",
      "https://github.com",
      "https://github.com/onlyone",
      "https://github.com/me/../../etc",
      "https://user:secret@github.com/me/cadre",
      "https://token@github.com/me/cadre",
      "ssh://bad%0auser@github.com/me/cadre",
      "https://github.com/me/cadre?x=1",
      "https://github.com/me/cadre#frag",
      "https://github.com/me/rak azo",
      "--upload-pack=touch /tmp/pwned",
      "-o ProxyCommand=id",
      `https://github.com/me/cadre${"x".repeat(400)}`,
    ]) {
  ...
  it("rejects an embedded newline that could smuggle a second argument", () => {
    expect(normalizeRepoUrl("https://github.com/me/cadre\n--exec=id")).toHaveProperty("error");
  });
});

describe("repoIdentity", () => {
  it("treats every spelling of the same remote as one repository", () => {
    const identity = "github.com/mosnin/cadre";
    expect(repoIdentity("https://github.com/mosnin/cadre")).toBe(identity);
    expect(repoIdentity("https://github.com/mosnin/cadre.git")).toBe(identity);
    expect(repoIdentity("git@github.com:mosnin/cadre.git")).toBe(identity);
    expect(repoIdentity("ssh://git@github.com/mosnin/cadre")).toBe(identity);
    expect(repoIdentity("not a url")).toBeNull();
  });

  it("only calls the real upstream official", () => {
    expect(isOfficialRepoUrl("git@github.com:mosnin/cadre.git")).toBe(true);
    expect(isOfficialRepoUrl("https://github.com/attacker/cadre")).toBe(false);
    expect(isOfficialRepoUrl("https://githubb.com/mosnin/cadre")).toBe(false);
  });
});
...
describe("updateSteps", () => {
  ...
  it("passes every value as its own argument so nothing reaches a shell", () => {
    const steps = updateSteps({
      remoteUrl: "git@github.com:me/cadre.git",
      branch: "release/1.2",
      targetCommit: "2".repeat(40),
      repointRemote: true,
    });
    expect(steps[0]).toEqual({
      ...
      command: "git",
      args: ["remote", "set-url", "origin", "git@github.com:me/cadre.git"],
    });
