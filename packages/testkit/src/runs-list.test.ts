...
import type { RunActivityRow } from "@cadre/contracts";
...
describeRunsList("runs.list activity tracker", () => {
  ...
  const dataDir = mkdtempSync(path.join(tmpdir(), "cadre-runs-list-"));
  ...
  it("lists active runs from two bots and keeps completed runs under recent", async () => {
    const cookie = await signup(app, `runs-${stamp}@cadre.test`, "Runs User");
  ...
  it("never returns runs from another workspace", async () => {
    const ownerCookie = await signup(app, `runs-owner-${stamp}@cadre.test`, "Owner");
    ...
    const intruderCookie = await signup(app, `runs-intruder-${stamp}@cadre.test`, "Intruder");
  ...
  it("excludes archived bots from recent before applying the limit", async () => {
    const cookie = await signup(app, `runs-archived-${stamp}@cadre.test`, "Archive User");
  ...
  it("returns group runs with groupId for navigation", async () => {
    const cookie = await signup(app, `runs-group-${stamp}@cadre.test`, "Group User");
