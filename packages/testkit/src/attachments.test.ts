...
describeAttachments("chat attachments", () => {
  ...
  const dataDir = mkdtempSync(path.join(tmpdir(), "cadre-attachments-"));
  ...
  it("uploads, sends with image/file, and rejects invalid attachments", async () => {
    const cookie = await signup(app, `attachments-${stamp}@cadre.test`, "Attachment User");
  ...
  it("attaches a workspace file into the thread", async () => {
    const cookie = await signup(app, `attach-thread-${stamp}@cadre.test`, "Attach Thread User");
    const bot = await rpc<{ id: string }>(app, cookie, "bots/create", {
      name: "Attacher",
      title: "Attacher",
