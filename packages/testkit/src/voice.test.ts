...
import { SCRIPTED_MPEG, SCRIPTED_TRANSCRIPT, SCRIPTED_VOICE_ID } from "@cadre/adapters";
...
describeVoice("voice credentials and speech HTTP", () => {
  ...
  const dataDir = mkdtempSync(path.join(tmpdir(), "cadre-voice-"));
  ...
  it("connects a scripted key, speaks, and transcribes without leaking the secret", async () => {
    const cookie = await signup(app, `voice-${stamp}@cadre.test`, "Voice User");
    const other = await signup(app, `voice-other-${stamp}@cadre.test`, "Other Voice");
