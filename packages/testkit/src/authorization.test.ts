...
import { ComposioEmulator } from "@cadre/adapters";
import type { appContract, Space, SpaceNavigation } from "@cadre/contracts";
...
describeWithDatabase("API authorization and resource isolation", () => {
  ...
  const dataDir = mkdtempSync(path.join(tmpdir(), "cadre-authz-"));
  ...
  it("rejects cross-origin authenticated mutations before changing state", async () => {
    const cookie = await signup(app, `origin-${stamp}@cadre.test`, "Origin test");
  ...
  it("prevents one user from reading or mutating another user's resources", async () => {
    const owner = await signup(app, `owner-authz-${stamp}@cadre.test`, "Authorization Owner");
    const intruder = await signup(
      app,
      `intruder-authz-${stamp}@cadre.test`,
  ...
  it("keeps approval rules private to each user in a shared Space", async () => {
    const owner = await signup(app, `approval-owner-${stamp}@cadre.test`, "Approval Owner");
    const member = await signup(app, `approval-member-${stamp}@cadre.test`, "Approval Member");
    const ownerActor = await rpc<Actor>(app, owner, "me");
  ...
  it("keeps space data and computers behind the selected space boundary", async () => {
    const cookie = await signup(app, `spaces-${stamp}@cadre.test`, "Space Owner");
    ...
    const intruder = await signup(app, `spaces-intruder-${stamp}@cadre.test`, "Intruder");
  ...
  it("enforces the space limit across concurrent creation requests", async () => {
    const cookie = await signup(app, `space-limit-${stamp}@cadre.test`, "Space Limit");
  ...
  it("reuses provider credentials and copies their selections into a new Space", async () => {
    const cookie = await signup(app, `space-provider-copy-${stamp}@cadre.test`, "Provider Copy");
    const actor = await rpc<Actor>(app, cookie, "me");
  ...
  it("shares model credentials while keeping defaults private to each space", async () => {
    const cookie = await signup(app, `model-defaults-${stamp}@cadre.test`, "Model Defaults");
    const actor = await rpc<Actor>(app, cookie, "me");
  ...
  it("validates per-bot model overrides against connected providers and catalog", async () => {
    const cookie = await signup(app, `bot-model-${stamp}@cadre.test`, "Bot Model");
    const bot = await rpc<
      Bot & {
  ...
  it("chooses the newest duplicate provider credential when selecting a default", async () => {
    const cookie = await signup(app, `model-duplicates-${stamp}@cadre.test`, "Model Duplicates");
    const actor = await rpc<Actor>(app, cookie, "me");
  ...
  it("restricts deployment settings to the deployment owner", async () => {
    const owner = await signup(app, `deployment-owner-${stamp}@cadre.test`, "Deployment Owner");
    const other = await signup(app, `deployment-other-${stamp}@cadre.test`, "Deployment Other");
    const ownerActor = await rpc<Actor>(app, owner, "me");
    ...
    try {
      ...
      const disallowedSignup = await app.request("/api/auth/sign-up/email", {
        ...
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          email: `not-approved-${stamp}@cadre.test`,
          password: "password123",
