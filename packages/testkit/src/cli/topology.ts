...
import { isTerminal } from "@cadre/core";

const composeFile = path.resolve("infra/compose/docker-compose.topology.yml");
const keep = process.argv.includes("--keep");
const skipBuild = process.argv.includes("--skip-build");
const project = `cadre-topology-${process.pid}-${Date.now().toString(36)}`;
...
async function main() {
  ...
  const env = {
    ...
    TOPOLOGY_APP_IMAGE: process.env.TOPOLOGY_APP_IMAGE ?? "cadre/app:topology",
    TOPOLOGY_COMPUTER_IMAGE: process.env.TOPOLOGY_COMPUTER_IMAGE ?? "cadre/computer:topology",
  };
...
async function signup(baseUrl: string) {
  const response = await fetch(`${baseUrl}/api/auth/sign-up/email`, {
    ...
    body: JSON.stringify({
      email: `topology-${Date.now()}@cadre.test`,
      password: "password12",
      name: "Topology",
    }),
  });
...
async function waitForWorker(
  ...
  while (Date.now() - started < timeoutMs) {
    ...
    if (logs.includes("cadre worker ready")) return;
    await delay(500);
  }
...
function assertComputerNetworkIsolation(
  ...
  const managed = docker([
    ...
    computerId,
    "--format",
    '{{index .Config.Labels "cadre.managed"}}',
  ]).trim();
