...
async function main() {
  ...
  try {
    ...
    execSync("pnpm --filter @cadre/db generate", { stdio: "inherit", env: process.env });
    ...
    if (integration) {
      execSync(
        [
          ...
          "apps/api/src/codex-oauth.postgres.test.ts",
    ...
    const [
      ...
      { createApp },
    ] = await Promise.all([import("@cadre/adapters"), import("../../../../apps/api/src/app.ts")]);
