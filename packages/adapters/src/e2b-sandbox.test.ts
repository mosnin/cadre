...
describe("E2B computer backend", () => {
  it("revokes an extra display's control without starting or waiting for its view", async () => {
    const command = vi.fn(async (value: string) => {
      if (value.includes("CADRE_SCREEN_INDEX=")) {
        return { stdout: "CADRE_SCREEN_INDEX=1\n", stderr: "", exitCode: 0 };
      }
      if (value.includes("CADRE_SCREEN_PASSWORD=") || value.includes("flock 8")) {
  ...
  it("reconnects concurrent primary viewers without using the SDK's global VNC lifecycle", async () => {
    const command = vi.fn(async (value: string) => ({
      stdout: value.includes("CADRE_SCREEN_INDEX=")
        ? "CADRE_SCREEN_INDEX=0\n"
        : "CADRE_SCREEN_PASSWORD=savedkey\n",
      stderr: "",
      exitCode: 0,
    }));
  ...
  it("opens http(s) URLs through the named browser launcher", async () => {
    const command = vi.fn(async (value: string) => {
      if (value.includes("CADRE_SCREEN_INDEX=")) {
        return { stdout: "CADRE_SCREEN_INDEX=0\n", stderr: "", exitCode: 0 };
      }
    ...
    expect(open).toHaveBeenCalledWith("/home/user/cadre-home/notes/readme.md");
  });
  ...
  it("controls the desktop and exposes a portable workspace", async () => {
    ...
    const command = vi.fn(async (value: string, _options?: Record<string, unknown>) => {
      ...
      if (value.includes("CADRE_SCREEN_RELEASE=")) {
        return {
          stdout: "CADRE_SCREEN_RELEASE=0\n",
          stderr: "",
          exitCode: 0,
        };
      }
      if (value.includes("primary-view.lock")) {
        return { stdout: "CADRE_SCREEN_PASSWORD=screen-key\n", stderr: "", exitCode: 0 };
      }
    ...
    command.mockImplementation((value, options) => {
      ...
      return new Promise((resolve) => {
        finishStart = () =>
          resolve({
            stdout: "CADRE_SCREEN_PASSWORD=screen-key\n",
            stderr: "",
            exitCode: 0,
          });
      });
  ...
  it("gives Team bots distinct E2B screens and shared files", async () => {
    ...
    const command = vi.fn(async (value: string) => {
      ...
      if (screenKey && value.includes("CADRE_SCREEN_INDEX=")) {
        ...
        return {
          stdout: `CADRE_SCREEN_INDEX=${index}\n`,
          stderr: "",
          exitCode: 0,
        };
      }
      if (screenKey && value.includes("CADRE_SCREEN_RELEASE=")) {
        const index = screenSlots.get(screenKey);
        if (index === undefined) {
          return {
            stdout: "CADRE_SCREEN_RELEASE=missing\n",
            stderr: "",
            exitCode: 0,
          };
        }
        ...
        return {
          stdout: `CADRE_SCREEN_RELEASE=${index}\n`,
          stderr: "",
          exitCode: 0,
        };
      }
      if (value.includes("command -v Xvfb")) return { stdout: "", stderr: "", exitCode: 0 };
      if (value.includes("CADRE_SCREEN_PASSWORD=")) {
        return {
          stdout: "CADRE_SCREEN_PASSWORD=test-view-password\n",
          stderr: "",
          exitCode: 0,
        };
      }
      ...
      if (value.includes("primary-view.lock")) {
        return { stdout: "CADRE_SCREEN_PASSWORD=primary-key\n", stderr: "", exitCode: 0 };
      }
...
describe("sandbox-gone detection", () => {
  ...
  const alive = [
    ...
    new Error("Path /home/user/cadre-home/notes.md not found"),
