...
const savedAllowPublic = process.env.CADRE_OPENAI_COMPAT_ALLOW_PUBLIC;

afterEach(() => {
  if (savedAllowPublic === undefined) delete process.env.CADRE_OPENAI_COMPAT_ALLOW_PUBLIC;
  else process.env.CADRE_OPENAI_COMPAT_ALLOW_PUBLIC = savedAllowPublic;
});
...
describe("openai-compatible URL policy", () => {
  ...
  it("rejects public hosts unless explicitly allowed", () => {
    delete process.env.CADRE_OPENAI_COMPAT_ALLOW_PUBLIC;
