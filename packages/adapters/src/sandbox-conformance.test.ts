...
import type { ComputerRef, ProcessEvent, SandboxProvider } from "@cadre/adapter-kit";
...
describe("sandbox conformance", () => {
  ...
  it("desktop executor times out and kills descendants that inherited its pipes", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "cadre-desktop-timeout-"));
  ...
  it("desktop executor aborts and kills a running command", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "cadre-desktop-abort-"));
    const desktop = new DesktopSandboxProvider({ root });
  ...
  it("reuses one desktop machine per bot", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "cadre-desktop-reuse-"));
    const desktop = new DesktopSandboxProvider({ root });
  ...
  it("desktop file writes do not follow a final symlink outside the workspace", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "cadre-desktop-symlink-"));
    const desktop = new DesktopSandboxProvider({ root });
...
describe("docker sandbox", () => {
  ...
  const dataDir = mkdtempSync(path.join(tmpdir(), "cadre-docker-conformance-"));
  ...
  it("runs the same graphical command through the supervisor", async ({ skip }) => {
    ...
    spawned = spawn("pnpm", ["--filter", "@cadre/sandbox-supervisor", "start"], {
      cwd: root,
      env: {
        ...process.env,
        DATA_DIR: dataDir,
...
function hasAnySandboxImage() {
  try {
    execSync("docker image inspect cadre/computer:local", { stdio: "ignore", timeout: 8_000 });
    return true;
  } catch {
    return false;
  }
