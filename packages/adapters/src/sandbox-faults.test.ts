...
import type { PortableFile, SandboxProvider } from "@cadre/adapter-kit";
...
describe.each([
  ...
  [
    "desktop",
    async () => {
      const root = await mkdtemp(path.join(tmpdir(), "cadre-sandbox-faults-"));
...
async function providerSet(label: string): Promise<Array<[string, SandboxProvider]>> {
  const root = await mkdtemp(path.join(tmpdir(), `cadre-sandbox-transfer-${label}-`));
  temporaryRoots.push(root);
  return [
