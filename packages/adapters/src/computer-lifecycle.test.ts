...
import { clearThread, type PrismaClient, type ThreadEvents } from "@cadre/db";
...
describe("computer provisioning", () => {
  ...
  it("never stops a provider when archive invalidates its boot claim", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "cadre-provision-race-"));
  ...
  it.each([
    ...
    async ({ stage, rollbackFails }) => {
      const dataDir = await mkdtemp(path.join(tmpdir(), "cadre-reconnect-rollback-"));
  ...
  it("does not stop a pre-existing computer when reconnect setup fails", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "cadre-reconnect-unowned-"));
  ...
  it.each([
    ...
    async (next) => {
      const dataDir = await mkdtemp(path.join(tmpdir(), "cadre-provision-reconnect-update-"));
  ...
  it.each([
    ...
    async ({ fresh }) => {
      const dataDir = await mkdtemp(path.join(tmpdir(), "cadre-prepare-rollback-"));
      const ref = {
        id: "provider-1",
        botId: "bot-1",
        kind: "fake" as const,
  ...
  it.each([false, true])(
    ...
    async (stopFails) => {
      const dataDir = await mkdtemp(path.join(tmpdir(), "cadre-team-activation-rollback-"));
      const ref = {
        id: "provider-1",
        botId: "team-home",
  ...
  it("retains a fresh provider reference when rollback also fails", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "cadre-prepare-rollback-failure-"));
  ...
  it.each([false, true])(
    ...
    async (prepareFails) => {
      const dataDir = await mkdtemp(path.join(tmpdir(), "cadre-provision-reconnect-"));
      const ref = {
        id: "provider-1",
        botId: "bot-1",
        kind: "cloud" as const,
...
describe("computer replacement", () => {
  ...
  it("replaces a wedged computer and restores the durable home", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "cadre-replace-"));
    const homeRoot = await mkdtemp(path.join(tmpdir(), "cadre-replace-home-"));
  ...
  it("claims a stopped computer before teardown so concurrent replacements serialize", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "cadre-replace-stopped-claim-"));
    const updateMany = vi
      .fn()
      .mockResolvedValueOnce({ count: 1 })
      .mockResolvedValueOnce({ count: 1 })
  ...
  it("continues recover when checkpoint fails with an ordinary provider error", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "cadre-recover-checkpoint-"));
    const homeRoot = await mkdtemp(path.join(tmpdir(), "cadre-recover-checkpoint-home-"));
    const home = new LocalAgentHomeStore(homeRoot);
    const sandbox = new FakeSandboxProvider();
    const first = await sandbox.provision({ botId: "bot-1", homePath: dataDir }, context);
  ...
  it("aborts update when checkpoint fails with an ordinary provider error", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "cadre-update-checkpoint-"));
    const homeRoot = await mkdtemp(path.join(tmpdir(), "cadre-update-checkpoint-home-"));
    const home = new LocalAgentHomeStore(homeRoot);
    const sandbox = new FakeSandboxProvider();
    const first = await sandbox.provision({ botId: "bot-1", homePath: dataDir }, context);
    vi.spyOn(sandbox, "exportWorkspace").mockImplementation(() => {
