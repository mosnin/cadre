...
describe("DaytonaSandboxProvider", () => {
  it("implements the portable command, file, screen, and computer-use contract", async () => {
    ...
    expect(fixture.create).toHaveBeenCalledWith(
      expect.objectContaining({
        labels: { botId: "bot-a", cadre: "computer" },
  ...
  it("gives Team bots distinct Daytona previews without a second computerUse.start", async () => {
    ...
    fixture.executeCommand.mockImplementation(async (command: string) => {
      ...
      if (command.includes("CADRE_SCREEN_PASSWORD=")) {
        return {
          exitCode: 0,
          result: "CADRE_SCREEN_PASSWORD=test-view-password\n",
        };
      }
...
function createScreenRegistryResponder() {
  ...
  return (command: string): { exitCode: number; result: string } | undefined => {
    ...
    if (command.includes("CADRE_SCREEN_INDEX=")) {
      ...
      return { exitCode: 0, result: `CADRE_SCREEN_INDEX=${index}\n` };
    }
    if (command.includes("CADRE_SCREEN_RELEASE=")) {
      const index = slots.get(key);
      if (index === undefined) return { exitCode: 0, result: "CADRE_SCREEN_RELEASE=missing\n" };
      slots.delete(key);
      return { exitCode: 0, result: `CADRE_SCREEN_RELEASE=${index}\n` };
    }
    return undefined;
  };
}
