import type { ModelOAuthSignInMode, ThinkingLevel } from "@cadre/contracts";
...
function catalogBilling(
  ...
  if (providerId === LOCAL_PROVIDER_ID) {
    return "Runs on infrastructure configured by the deployment owner. No model charges from Cadre.";
  }
  if (providerId === OPENAI_COMPATIBLE_PROVIDER_ID) {
    return "Runs on a URL you control.";
  }
  if (opts.oauth && !opts.apiKey) {
    return `${name} subscription login is not in the Cadre UI yet. Skip if this deployment already has credentials.`;
  }
  if (opts.apiKey) {
    return `Uses your ${name} API key.`;
  }
