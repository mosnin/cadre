...
/** Seeded cadre releases so evals succeed without the public internet. */
export const DEFAULT_CADRE_EMULATED_RELEASES: readonly EmulatedGithubRelease[] = [
  {
    owner: "mosnin",
    repo: "cadre",
    tag: "v0.4.2",
    ...
    htmlUrl: "https://github.com/mosnin/cadre/releases/tag/v0.4.2",
  },
  {
    owner: "mosnin",
    repo: "cadre",
    ...
    htmlUrl: "https://github.com/mosnin/cadre/releases/tag/v0.4.1",
  },
];
...
export function assessReleaseWatchRoutinePrompt(prompt: string): {
  ...
  const mentionsRepo = /cadre|mosnin/i.test(text);
  ...
  if (!mentionsRepo || !mentionsReleases || !mentionsGithubTool) {
    const missing = [
      !mentionsRepo ? "repo (mosnin/cadre)" : null,
