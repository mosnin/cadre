...
const BACKGROUND_WORK_MARKER_PREFIX = "/tmp/cadre-background-";
const BACKGROUND_WORK_IDLE_SENTINEL = "cadre-background-idle";
...
export const CANCEL_COMPUTER_RUN_WORK = [
  ...
  'pkill -TERM -f "cadre-background-launch ${computerId} ${runId} " 2>/dev/null || true',
  "if [ -d /proc ]; then",
  ...
  "fi",
  "sleep 0.2",
  'pkill -KILL -f "cadre-background-launch ${computerId} ${runId} " 2>/dev/null || true',
  "if [ -d /proc ]; then",
  "  for fd in /proc/[0-9]*/fd/*; do",
  '    target="$(readlink "$fd" 2>/dev/null)" || continue',
  '    case "$target" in',
...
export function cancelComputerRunWorkArgv(computerId: string, runId: string): string[] {
  return ["bash", "-c", CANCEL_COMPUTER_RUN_WORK, "cadre-cancel-run-work", computerId, runId];
}
...
async function hasActiveBackgroundWork(
  ...
  try {
    for await (const event of sandbox.execute(
      computer,
      {
        argv: ["bash", "-c", BACKGROUND_WORK_PROBE, "cadre-background-probe", computerId],
        timeoutMs: 10_000,
      },
      context,
    )) {
