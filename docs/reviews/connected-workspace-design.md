...
The final messaging review covers a ninth surface: the phone peer-delivery transcript. The scripted runtime no longer echoes internal routing instructions. Real-agent instructions now require outcome-led replies, normal capitalization, teammate names instead of routing IDs, and evidence for completion claims. This verifies the fixture and instruction path, not every response from a real model.
