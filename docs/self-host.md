...
```bash
export CADRE_DOWNLOAD_BASE=https://example.com/mirror/cadre/infra/compose
bash install-images.sh
```

Trailing slashes on `CADRE_DOWNLOAD_BASE` are trimmed; non-HTTPS bases are rejected. Downloads
...
`CADRE_DOWNLOAD_SKIP_EXISTING=1` and/or pass `--local`:

```bash
# after placing docker-compose.images.yml and .env.images.example locally
bash install-images.sh --local --prepare-only
# or
CADRE_DOWNLOAD_SKIP_EXISTING=1 bash install-images.sh --prepare-only
```
...
```bash
export CADRE_INSTALLER_URL=https://example.com/mirror/cadre/infra/compose/install-images.sh
mkdir -p cadre && cd cadre &&
curl -fsSLO "${CADRE_INSTALLER_URL}" &&
bash install-images.sh
```
...
`CADRE_HOST` to its hostname (for example, `app.example.com`).
...
Compose runs Postgres, the sandbox supervisor (Docker socket), API, worker, and a Vite preview of the web app. Bot computers are sibling containers (`cadre/computer:local`) on separate per-bot networks; only the supervisor and screen proxy join each one. The API process does not get an unrestricted Docker socket; the supervisor owns the lifecycle.
...
appears on sign-in only when a transactional email provider is available. Cadre uses a
...
EMAIL_FROM=Cadre <no-reply@example.com>
```
...
STARTTLS. Cadre rejects configuration that disables TLS or certificate verification.
...
Services set `service.name` (`cadre-api`, `cadre-worker`, `cadre-sandbox-supervisor`,
`cadre-updater`). A partial Axiom config logs a one-time warning and stays off. `AXIOM_EDGE` is a
...
CADRE_COMPUTER_MEMORY_MB=4096 # memory cap for each Docker bot computer; minimum 512
...
```env
CADRE_LOCAL_MODELS=qwen3:4b,llama3.1:8b
CADRE_LOCAL_MODELS_URL=http://127.0.0.1:11434/v1
CADRE_LOCAL_CONTEXT_WINDOW=32768
CADRE_LOCAL_MAX_TOKENS=4096
```

The loopback default is suitable when running Cadre from a source checkout. In Docker Compose,
...
leave Cadre through this URL. Leave `CADRE_LOCAL_MODELS` blank to disable the provider.
...
the exact model id from that server, and an optional API key. By default Cadre only allows
loopback, RFC1918, and `host.docker.internal` targets. To permit public hostnames, set
`CADRE_OPENAI_COMPAT_ALLOW_PUBLIC=1` in the deployment environment. Public hostnames must resolve
...
service. Cadre's own fetch path already refuses link-local and metadata addresses, but a page
...
can. This is a host control; Cadre cannot enforce it from inside the container.
## Typed decisions
...
Cadre uses TypeSafe's Jev through OpenRouter's Decisions API. It reuses `OPENROUTER_API_KEY`,
...
- **E2B** runs bot computers away from the Cadre host and is a good choice for public or multi-user
  production deployments. Cadre checkpoints the portable workspace and browser-profile directory to
...
- **Box by ASCII** provides a managed Linux desktop through `BOX_API_KEY` and optionally
  `BOX_API_URL`. Cadre always creates or resumes boxes with `noEnv: true`, keeps the portable
  workspace under `/home/user/cadre-home`, and refreshes a two-hour TTL. A Box currently exposes one
...
2. Clone the repository on the VM and create a root `.env` with production-only values. At minimum set
   ...
   `CADRE_HOST`, and the three public origins. Set `CADRE_DEPLOY_DIR` when the checkout is not at
   the supported Linux default, `/srv/cadre`. Use URL-safe random values for database credentials.
   If you enable the `updater` profile, also set a dedicated `CADRE_UPDATER_TOKEN` (at least 32
...
WAKEUP_DRIVER=graphile
DATA_DIR=/data
# Absolute path of this checkout as the Docker daemon sees it. /srv/cadre is the Linux default;
...
# CADRE_UPDATER_TOKEN=replace-with-32-plus-character-updater-token
```
...
**Build, do not pull, for a first deployment.** `CADRE_IMAGE_TAG` ships as `local`, a tag no
...
`CADRE_IMAGE_TAG` to that release tag and running `pull` followed by `up -d --wait --pull never`.
...
`/usr/local/sbin/cadre-backup` and enable the supplied `cadre-backup.timer`. It creates a verified
Postgres custom-format dump plus an application-data archive under `/var/backups/cadre`, with mode
...
`CADRE_IMAGE_TAG` (or rebuild `local`) and `up -d --wait --pull never`.

The updater sidecar has its own image and tag so an update never recreates the process performing
it. Move it deliberately by setting `CADRE_UPDATER_IMAGE_TAG` to the full `sha-<commit>` tag, then
...
`GIT_SHA=$(git rev-parse HEAD)`, run `pnpm --filter @cadre/db migrate`, then restart API and worker.
...
also pull the same app tags once `CADRE_IMAGE_TAG` is set to a published value.

If you deploy from your own fork, set `CADRE_IMAGE` and `CADRE_UPDATER_IMAGE` to your namespace —
...
content addressing can pin `CADRE_IMAGE` outside the automatic updater to a verified digest.
...
`CADRE_UPDATER_TOKEN` and explicitly start the profile:
...
`CADRE_UPDATER_TOKEN`. Operator CLI upgrades above do not need it; the sidecar is for automated
...
- *Official repository:* resolves the newest stable release and its source commit with
  ...
  outgoing tag in `CADRE_IMAGE_TAG_PREVIOUS`, explicitly pulls the new image, then runs
...
- *Fork (Advanced):* a fork has no published images, so the sidecar fast-forwards the checkout in
  `CADRE_DEPLOY_DIR` and runs `up -d --build`. This builds on the server and takes minutes rather
...
`CADRE_DEPLOY_DIR` is bind-mounted into the updater at the same path it is read from
(`${CADRE_DEPLOY_DIR}:${CADRE_DEPLOY_DIR}`), and that is load-bearing rather than tidy. Production
Compose defaults both sides to `/srv/cadre`; set the variable for any other layout. When the
updater runs `docker compose -p <project> --file $CADRE_DEPLOY_DIR/infra/compose/docker-compose.prod.yml up -d`,
...
`CADRE_COMPOSE_PROJECT_NAME`; the final fallback is `cadre-prod`. Without that propagation, a
...
- **Linux.** The daemon shares the host filesystem, so the checkout path is the answer:
  `/srv/cadre` is the default and supported production layout. Set `CADRE_DEPLOY_DIR` explicitly
  when the checkout is elsewhere.
- **Docker Desktop (Windows/macOS).** The daemon runs in a VM that mounts your drive somewhere else.
  On Windows, `C:` appears at `/run/desktop/mnt/host/c`, so a checkout at `C:\Users\you\cadre` is
  `CADRE_DEPLOY_DIR=/run/desktop/mnt/host/c/Users/you/cadre`. Host Git may use `core.autocrlf=true`; the updater ignores CR-only diffs so that does not block `/apply`. Verify the mount before deploying:

```bash
docker compose --env-file .env -f infra/compose/docker-compose.prod.yml \
  --profile updater run --rm updater git -C "$CADRE_DEPLOY_DIR" log --oneline -1
```
...
- The process environment carries only updater settings (`CADRE_UPDATER_TOKEN`, deploy path,
...
Enabling the `updater` profile requires `CADRE_UPDATER_TOKEN` to be a dedicated random value (at
...
10. Turn on `SIGNUP_ALLOWLIST` until you want open registration. There is no Cadre-managed model billing in version 1 — users bring keys.

Expo / desktop installers are clients of that origin (`EXPO_PUBLIC_API_URL`, `CADRE_WEB_URL`). They are not a Cloud control plane.
