...
| --- | --- | --- | --- | --- | --- |
| Operate | `https://operate.to` | dynamic, `/oauth/register` | `openid email operate:read operate:write` | `/api/mcp` | `org_id`, `org_name` |
| Stored | `https://www.stored.to` | seeded client id (`STORED_OAUTH_CLIENT_ID`) | `openid profile org:read memory:read memory:write` | `/api` | `org_id`, `org_name` |
| Scalar | `https://www.tryscalar.xyz` | dynamic, `/oauth/register` | `openid profile crm:read crm:write mcp` | `/api/mcp/mcp` | `workspace.id`, `workspace.name` (or `org_id`) |
| Company OS | configured | seeded confidential client | capabilities | `/api/mcp` | `company.id`, `company.name`, `company.slug` |

`packages/adapters/src/workspace-integrations.ts` holds the table; origins and the Stored client id can be overridden per deployment through the environment.
...
- Stored: seed an OAuth client for Cadre with the callback `https://<cadre-web>/api/v1/workspace-integrations/stored/callback`; accept OAuth bearer tokens on the organization MCP endpoint and on a Cadre memory endpoint (`POST /api/cadre/v1/memory` with `action: save | recall | purgeHistory`, `workspace`, `botId`); add the per-agent private memory endpoint `/mcp/cadre/<workspaceId>/<botId>`; record which Cadre workspace an organization is linked to.
