...
PROFILE="${CADRE_BROWSER_PROFILE:-$AGENT_HOME/.browser-profiles/chromium}"
export CADRE_BROWSER_PROFILE="$PROFILE"
mkdir -p "$AGENT_HOME" "$AGENT_HOME/.local/bin" "$AGENT_HOME/.config" /tmp/cadre /tmp/.X11-unix ${FLUX_HOME}
...
LOG_DIR="/tmp/cadre/display-$DISPLAY_NUMBER"
mkdir -p "$LOG_DIR"
...
if [[ -n "${CADRE_COMPUTER_CONTROL_TOKEN:-}" ]]; then
  /usr/local/bin/cadre-computer-control >>"$LOG_DIR"/control.log 2>&1 &
fi
...
cp /etc/cadre/fluxbox/init ${FLUX_HOME}/.fluxbox/init
cp /etc/cadre/fluxbox/apps ${FLUX_HOME}/.fluxbox/apps 2>/dev/null || true
cp /etc/cadre/fluxbox/menu ${FLUX_HOME}/.fluxbox/menu 2>/dev/null || true
...
tint2 -c /etc/cadre/tint2rc >>"$LOG_DIR"/dock.log 2>&1 &

register_browser_handler() {
  local mime="$1"
  if ! xdg-mime default cadre-browser.desktop "$mime" >/dev/null 2>&1 \
    || [[ "$(xdg-mime query default "$mime" 2>/dev/null || true)" != "cadre-browser.desktop" ]]; then
    echo "failed to register cadre-browser for $mime" >&2
    exit 1
  fi
}
...
if ! xdg-settings set default-web-browser cadre-browser.desktop >/dev/null 2>&1 \
  || [[ "$(xdg-settings get default-web-browser 2>/dev/null || true)" != "cadre-browser.desktop" ]]; then
  echo "failed to set default web browser to cadre-browser" >&2
  exit 1
fi
...
HOME="$AGENT_HOME" cadre-browser >>"$LOG_DIR"/browser.log 2>&1 &
browser_up=0
...
start_view_proxy() {
  websockify --heartbeat=30 --web="$NOVNC_ROOT" "${CADRE_VNC_HOST:-0.0.0.0}:$VIEW_PORT" "127.0.0.1:$VIEW_VNC_PORT" >>"$LOG_DIR"/novnc.log 2>&1 &
}
