...
export function resolveUpdaterConfig(env: NodeJS.ProcessEnv): UpdaterConfig {
  const deployDir = env.CADRE_DEPLOY_DIR?.trim() ?? "";
  if (deployDir === "" || !path.posix.isAbsolute(deployDir)) {
    throw new Error("Set CADRE_DEPLOY_DIR to the absolute path of the deployment directory.");
  }
  const image = env.CADRE_IMAGE?.trim() || OFFICIAL_SERVER_IMAGE;
  if (!isValidImageName(image)) throw new Error(`CADRE_IMAGE is not a usable image name: ${image}`);
  const composeFile = env.CADRE_COMPOSE_FILE?.trim() || DEFAULT_COMPOSE_FILE;
  if (path.posix.isAbsolute(composeFile) || composeFile.split("/").includes("..")) {
    throw new Error("CADRE_COMPOSE_FILE must be a path inside the deployment directory.");
  }
  const port = Number(env.CADRE_UPDATER_PORT ?? DEFAULT_UPDATER_PORT);
  if (!Number.isInteger(port) || port <= 0 || port > 65_535) {
    throw new Error("CADRE_UPDATER_PORT is not a port number.");
  }
  return {
    deployDir,
    ...
    host: env.CADRE_UPDATER_HOST?.trim() || "127.0.0.1",
    port,
  };
}
