...
const FIRST_PARTY_IMAGE = /ghcr\.io\/mosnin\/cadre\/([a-z0-9][a-z0-9._-]*)/g;
...
describe("the images compose file", () => {
  it("runs postgres, app roles, supervisor, and a published computer image", () => {
    ...
    for (const service of appServices) {
      expect(compose.services[service]?.image).toContain("ghcr.io/mosnin/cadre/app");
      expect(compose.services[service]?.image).toContain("CADRE_IMAGE_TAG");
    }
    expect(compose.services.computer?.image).toContain("ghcr.io/mosnin/cadre/computer");
  ...
  it("skips non-string Compose environment scalars when collecting image names", () => {
    ...
    expect(firstPartyImageNames("ghcr.io/mosnin/cadre/computer:edge")).toEqual(["computer"]);
  });
